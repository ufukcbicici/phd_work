...
conv_x = np.random.uniform(size=(batch_size_param, 16, 16, 32))
...
conv_input = tf.keras.Input(shape=(16, 16, 32), name="conv_input")
conv_net = conv_input
conv_net = tf.keras.layers.Conv2D(64, 3, padding="same")(conv_net)
conv_net = tf.keras.layers.BatchNormalization()(conv_net)
conv_net = tf.keras.layers.LeakyReLU(alpha=0.2)(conv_net)
conv_output = tf.keras.layers.MaxPool2D(pool_size=2, strides=2)(conv_net)

# conv_x = np.random.uniform(size=(batch_size_param, 16, 16, 32))
# model_conv = tf.keras.Model(inputs={"conv_input": conv_input},  outputs=[conv_output])
# results = model_conv(inputs={"conv_input": conv_x}, training=False)
# print("X")
...
model = tf.keras.Model(inputs={"tf_x": tf_x, "tf_router": tf_router,
                               "tf_temperature": tf_temperature, "conv_input": conv_input},
...
for trial_id in range(trial_count):
    ...
    conv_x = np.random.uniform(size=(batch_size_param, 16, 16, 32))
    routing_values = np.random.uniform(size=(batch_size_param, node_degree), low=-10.0, high=10.0)
    results = model(inputs={"tf_x": x,
                            "tf_temperature": 5.0,
                            "tf_router": routing_values,
                            "conv_input": conv_x},
                    training=False)
