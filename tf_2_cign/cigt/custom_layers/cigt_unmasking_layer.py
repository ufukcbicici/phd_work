...
# OK
class CigtUnmaskingLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        x_ = inputs[0]
        regular_routing_matrix = inputs[1]
        # routing_matrix_zero_division_safe = tf.where(tf.equal(routing_matrix, 0), 1e-20, routing_matrix)
        routing_matrix = tf.math.reciprocal_no_nan(regular_routing_matrix)
