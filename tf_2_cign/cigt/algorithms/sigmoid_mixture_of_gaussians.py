...
class SigmoidMixtureOfGaussians:
    ...
    def sample(self, num_of_samples):
        ...
        s = self.gaussianMixture.sample(n_samples=num_of_samples)
        s2 = 1.0 + np.exp(-s)
        y = np.reciprocal(s2)
    ...
    def pdf(self, y):
        ...
        for comp_idx in range(self.numOfComponents):
            ...
            pdf_x = norm.pdf(x=x, loc=component_mean, scale=component_variance)
        ...
        res = np.sum(component_densities, axis=1)
        res = res * a
        return res
...
def plot_and_integrate():
    sigmoid_normal = SigmoidMixtureOfGaussians(num_of_components=2)
