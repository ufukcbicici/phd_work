...
class FashionCignV2(FastTreeNetwork):
    ...
    def root_func(self, node, network, variables=None):
        # Parameters
        ...
        # ***************** H: Connected to F *****************
        self.apply_router_transformation(net=net, network=network, node=node,
                                         decision_feature_size=GlobalConstants.FASHION_NO_H_FROM_F_UNITS_1)
        # ***************** H: Connected to F *****************

    def l1_func(self, node, network, variables=None):
        # Parameters
        node_degree = network.degreeList[node.depth]
        # Convolution 2
        # OK
    ...
    def leaf_func(self, node, network, variables=None):
