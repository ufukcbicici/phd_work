...
def batch_norm(x, network, node, decay, iteration, is_decision_phase, is_training_phase):
    ...
    last_mean = tf.Variable(name=last_mean_name, initial_value=tf.constant(0.0, shape=[x.get_shape()[-1]]),
                            trainable=False)
    last_var = tf.Variable(name=last_var_name, initial_value=tf.constant(0.0, shape=[x.get_shape()[-1]]), trainable=False)
    ...
    normed_x = tf.nn.batch_normalization(x=x, mean=final_mean, variance=final_var, offset=beta, scale=gamma, variance_epsilon=1e-5)
