...
class Utilities:
    ...
    def get_iou_with_list(bb, bb_y_matrix):
        ...
        intersection_widths = np.clip(intersection_right - intersection_left, a_min=0)
        intersection_heights = np.clip(intersection_bottom - intersection_top, a_min=0)
        intersection_areas = intersection_widths * intersection_heights
        bb_x_areas = (bb_x_matrix[:, 3] - bb_x_matrix[:, 0]) * (bb_x_matrix[:, 4] - bb_x_matrix[:, 1])
        bb_y_areas = (bb_y_matrix[:, 3] - bb_y_matrix[:, 0]) * (bb_y_matrix[:, 4] - bb_y_matrix[:, 1])
    ...
    def get_iou_of_bbs(bb_x, bb_y):
        ...
        intersection_left = np.max(bb_x[0], bb_y[0])
        intersection_right = np.min(bb_x[2], bb_y[2])
        intersection_top = np.max(bb_x[1], bb_y[1])
        intersection_bottom = np.min(bb_x[3], bb_y[3])
